...
#ethnicity -- should do more on this
...
# Figure 1, Table 2 & Accompanying Analyses
...
df_ext_usens_conf <- df_ext_usens_conf %>% 
  mutate(ext_dir = case_when(
    ...
    conf_dir_text = factor(conf_dir_text, 
                           levels = c("Very sure exists", "Pretty sure exists", "Somewhat sure exists", "Not sure", "Somewhat sure doesn't exist", "Pretty sure doesn't exist", "Very sure doesn't exist"))
    )
```
...
```{r, fig.width=15}
conf_colors <- rev(RColorBrewer::brewer.pal(7, "PuBu"))

# Create stacked bar plot  
df_ext_usens_conf %>% 
  filter(!is.na(conf_dir_text)) %>% 
  mutate(category = factor(category, levels = c("Fictional", "Supernatural", "Scientific", "Ordinary"))) %>%
  ...
  theme(axis.text.x = element_text(size = 14, angle = -35, vjust = -0.5),
...
Models predicting confidence x existence scores by category with separate regressions for each country. The pairwise contrasts here showing scientific vs. supernatural are what make up Table 2. 
...
# Ecuador
m.ecuador <- lmer(conf_dir ~ question_type + (1|ont_subj), data = filter(df_ext_usens_conf, ont_ctry_name == "Ecuador_Amazon"))
summary(m.ecuador)
...
# Peru
m.peru <- lmer(conf_dir ~ question_type + (1|ont_subj), data = filter(df_ext_usens_conf, ont_ctry_name == "Peru_Amazon"))
summary(m.peru)
...
data_long_comm %>%
  ...
  labs(
    ...
    y = "Proportion of Responses",  
    fill = "Responses",  
    title = "Can people communicate with __?\nby Category and Country"
  ) +
  theme(axis.text.x = element_text(size = 14, angle = -35, vjust = -0.5),  
        axis.title.x = element_text(size = 15),
        axis.text.y = element_text(size = 16),
...
# Ecuador
m.comm.ecuador <- lmer(response ~ category + (1|ont_subj), data = filter(data_long_comm, ont_ctry_name == "Ecuador_Amazon"))
...
# Peru
m.comm.peru <- lmer(response ~ category + (1|ont_subj), data = filter(data_long_comm, ont_ctry_name == "Peru_Amazon"))
summary(m.comm.peru)
...
```{r, fig.width=15}
data_long_spec <- df %>%
  filter(ont_ctry_name %in% c("US", "Ecuador_Amazon", "Peru_Amazon")) %>%  #question wasn't asked in ghana
    select(ont_ctry_name, ont_subj,
...
# Ecuador
m.spec.ecuador <- lmer(response ~ category + (1|ont_subj), data = filter(data_long_spec, ont_ctry_name == "Ecuador_Amazon"))
...
# Peru
m.spec.peru <- lmer(response ~ category + (1|ont_subj), data = filter(data_long_spec, ont_ctry_name == "Peru_Amazon"))
summary(m.spec.peru)
...
combined_sens %>%
  ...
  geom_text(aes(label = scales::percent(proportion_yes, accuracy = 1)),
            position = position_dodge(width = 0.8), vjust = 2, size = 5) +
  labs(
    title = "Can __ be sensed?: Proportion of 'Yes' Responses by Country",
    x = "Country",
...
combined_usens %>%
  ...
  geom_text(aes(label = scales::percent(proportion_yes, accuracy = 1)),
            position = position_dodge(width = 0.8), vjust = 2, size = 5) +
  labs(
    title = "Have you ever sensed __?: Proportion of 'Yes' Responses by Country",
    x = "Country",
    y = "Proportion of 'Yes' Responses"
  ) + 
  theme(axis.text.x = element_text(size = 14),
...
```{r}
# Version 1 -- 10 participants 
yn_stacked_bar(country = "Ecuador_Amazon", version = 1, 
...
# Version 2 -- 10 participants
yn_stacked_bar(country = "Ecuador_Amazon", version = 2, 
...
# Both versions together
yn_by_category(country = "Ecuador_Amazon", 
                 cols = c("sci_comu_flu", "spn_comu_wen", "ord_comu_mug", "sci_comu_par", "fic_comu_spd",
                          "sci_comu_cnc", "spn_comu_god", "ord_comu_tbl", "spn_comu_okw", "fic_comu_ram"),
...
```{r}
# Version 1 -- 21 participants
yn_stacked_bar(country = "Peru_Amazon", version = 1, 
...
# Version 2 -- 19 participants
yn_stacked_bar(country = "Peru_Amazon", version = 2, 
...
# Both versions together
yn_by_category(country = "Peru_Amazon", 
                 cols = c("spn_comu_ysh", "sci_comu_par", "spn_comu_god", "ord_comu_tbl", 
                          "fic_comu_njt", "spn_comu_cha", "sci_comu_cnc",
                          "sci_comu_grm", "nas_comu_ron", "ord_comu_taz", "spn_comu_oni", 
                          "fic_comu_smn", "nas_comu_dlp"),
...
```{r}
# Version 1 -- 10 participants 
spexp_stacked_bar(country = "Ecuador_Amazon", version = 1, 
...
# Version 2 -- 10 participants
spexp_stacked_bar(country = "Ecuador_Amazon", version = 2, 
...
# Both versions together
spexp_by_category(country = "Ecuador_Amazon", 
                 cols = c("sci_spexp_flu", "spn_spexp_wen", "ord_spexp_mug", "sci_spexp_par", "fic_spexp_spd",
                          "sci_spexp_cnc", "spn_spexp_god", "ord_spexp_tbl", "spn_spexp_okw", "fic_spexp_ram"),
...
```{r}
# Version 1 -- 21 participants
spexp_stacked_bar(country = "Peru_Amazon", version = 1, 
...
# Version 2 -- 19 participants
spexp_stacked_bar(country = "Peru_Amazon", version = 2, 
...
# Both versions together
spexp_by_category(country = "Peru_Amazon", 
                 cols = c("spn_spexp_ysh", "sci_spexp_par", "spn_spexp_god", "ord_spexp_tbl", 
                          "fic_spexp_njt", "spn_spexp_cha", "sci_spexp_cnc",
                          "sci_spexp_grm", "nas_spexp_ron", "ord_spexp_taz", "spn_spexp_oni", 
                          "fic_spexp_smn", "nas_spexp_dlp"),
