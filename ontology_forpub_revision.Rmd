...
Graphing Figure 1--revised version does not include fictional objects
...
Revised version also removes fictional objects

```{r}
# 1 regression per country (4 total) where response is predicted by category

# US
m.us <- lmer(conf_dir ~ question_type + (1|ont_subj), 
             data = filter(df_ext_usens_conf %>% filter(category != "Fictional"), 
                           ont_ctry_name == "US"))
summary(m.us)
...
# Ghana
m.ghana <- lmer(conf_dir ~ question_type + (1|ont_subj), 
                data = filter(df_ext_usens_conf %>% filter(category != "Fictional"), 
                              ont_ctry_name == "Ghana"))
summary(m.ghana)
...
# Ecuador
m.ecuador <- lmer(conf_dir ~ question_type + (1|ont_subj), 
                  data = filter(df_ext_usens_conf %>% filter(category != "Fictional"), 
                                ont_ctry_name == "Ecuador"))
summary(m.ecuador)
...
# Peru
m.peru <- lmer(conf_dir ~ question_type + (1|ont_subj), 
               data = filter(df_ext_usens_conf %>% filter(category != "Fictional"), 
                             ont_ctry_name == "Peru"))
summary(m.peru)
...
m.non.us <- lmer(conf_dir ~ question_type + (1|ont_ctry_name/ont_subj), 
                 data = filter(df_ext_usens_conf, ont_ctry_name != "US") %>% filter(category != "Fictional"))
summary(m.non.us)
...
For revised paper, Fig 2 now does not include Fictional objects
...
# US
m.comm.us <- lmer(response ~ category + (1|ont_subj), 
...
# Ghana
m.comm.ghana <- lmer(response ~ category + (1|ont_subj), 
...
# Ecuador
m.comm.ecuador <- lmer(response ~ category + (1|ont_subj), 
...
# Peru
m.comm.peru <- lmer(response ~ category + (1|ont_subj), 
...
# US
m.spec.us <- lmer(response ~ category + (1|ont_subj), 
...
# Ecuador
m.spec.ecuador <- lmer(response ~ category + (1|ont_subj), 
...
# Peru
m.spec.peru <- lmer(response ~ category + (1|ont_subj), 
...
combined_sens %>%
  filter(question_type != "Fictional") %>%
  ...
  theme_bw() +
  geom_text(aes(label = scales::label_number(accuracy = 1)(proportion_yes * 100)),
...
m.conf <- lmer(conf_dir ~ consensus_excl_self + usens_response + (1 | ont_ctry_name/ont_subj), 
              data = df_ext_usens_conf %>% filter(category != "Fictional"))
summary(m.conf)
